...
def extract_timestamp(filename):
    """Extract timestamp from filename, assuming format 'resv2_YYYYMMDD_HHMMSS.json'."""
    try:
        # Split by '_' and take the timestamp part before '.json'
        timestamp_str = filename.split('_')[1].split('.')[0]
        return datetime.strptime(timestamp_str, '%Y%m%d_%H%M%S')
    except (IndexError, ValueError):
        # If parsing fails, return earliest possible time
        return datetime.min

def find_latest_report(dir_path='/app/data', prefix='resv2_'):
    """
    Return the JSON file with the latest timestamp in its name matching <prefix>*.json
    ...
    if not files:
        return None
    # Select file with the latest timestamp in its name
    return max(files, key=lambda f: extract_timestamp(os.path.basename(f)))
